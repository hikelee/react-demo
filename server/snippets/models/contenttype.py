...
def init_contenttype():
    from snippets.models import ContentType, BaseModel
    contenttypes = {}
    for contenttype in ContentType.objects.all():
        contenttypes[contenttype.app + "-" + contenttype.name] = contenttype
    all_models = {}
    for model in [model for model in django.apps.apps.get_models() if issubclass(model, BaseModel)]:
...
def get_map():
    if not cache:
        with lock:
            ...
            cache['models'] = {model.get_contenttype_key(): model for model in django.apps.apps.get_models() if issubclass(model, BaseModel)}
    return cache
